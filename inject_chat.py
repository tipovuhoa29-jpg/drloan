...
# find send button (the circular one with send icon)
...
# 3. Replace all quick reply chips (which might have gotten showToast from previous script)
# First we find the container
...
if match:
    chips = match.group(1)
    # the previous script might have added onclick="showToast(...)". We forcefully replace them.
    chips_html = '''
...
<script>
    const responses = {
        'đau mỏi vai gáy': 'Tình trạng đau mỏi vai gáy thường do thoái hóa đốt sống cổ hoặc căng cơ do sai tư thế. Gối Gối F4/09 của DoctorLoan được sinh ra chính xác để nắn chỉnh lại đường cong sinh lý quanh cổ. Bạn có muốn xem thêm chi tiết không?',
        'chọn gối': 'Với dân phòng/người hay lái xe, bộ đôi Gối Cổ F4/09 và Đệm ngồi DoctorLoan là kết hợp tối ưu nhất để cân bằng cột sống. Bạn muốn tôi hướng dẫn cách mua hàng luôn chứ?',
        'hỏi giá': 'Đệm thiền DoctorLoan hiện đang có giá 1.200.000đ. Đệm giúp định hình xương chậu, hỗ trợ ngồi lâu không bị thoái hóa đốt sống lưng.',
        'kinh doanh': 'Với tư cách Đại lý, bạn sẽ nhận được chiết khấu từ 10% đến 25% tùy theo gói sản phẩm phân phối. Tôi có thể hỗ trợ bạn tư vấn quy trình tạo mã QR Affiliate nhé!',
    ...
    function appendBotTyping() {
        ...
        const typingHtml = 
            <div id="" class="flex items-end gap-2 max-w-[90%] md:max-w-[75%]">
...
print("Injected ChatGPT simulation logic")
